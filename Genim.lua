...
function Genim:CreateWindow(Config)
    ...
    if Config.KeySystem then
        ...
        local KeyInfo = KeySettings.Key or {}
        local KeyVerified = false
        
        -- Default Settings
        KeySettings.Title = KeySettings.Title or Config.Name .. " | Chave"
        KeySettings.Subtitle = KeySettings.Subtitle or "Sistema de Verificação"
        KeySettings.Note = KeySettings.Note or "Entre com a sua chave para continuar."
        KeySettings.SaveKey = KeySettings.SaveKey or false
        KeySettings.FileName = KeySettings.FileName or "GenimKey_" .. Config.Name
        KeySettings.GrabKeyFromSite = KeySettings.GrabKeyFromSite or false
        
        local KeySystemFrame = Create("Frame", {
            Name = "KeySystem",
            Parent = MainFrame,
            BackgroundColor3 = Genim.Theme.MainColor,
            BorderSizePixel = 0,
        ...
        Create("UICorner", { CornerRadius = UDim.new(0, 10), Parent = KeySystemFrame })
        ...
        Create("UICorner", { CornerRadius = UDim.new(0, 8), Parent = KeyInputFrame })
        Create("UIStroke", { Color = Genim.Theme.StrokeColor, Thickness = 1, Parent = KeyInputFrame })
        ...
        local ButtonContainer = Create("Frame", {
            Parent = KeySystemFrame,
            BackgroundTransparency = 1,
            Position = UDim2.new(0.5, -150, 0, 205),
        ...
        Create("UIStroke", { Color = Genim.Theme.StrokeColor, Thickness = 1, Parent = GetBtn })
        ...
        local function Shake(obj)
            ...
            task.spawn(function()
                for i = 1, 6 do
                    Tween(obj, 0.05, {Position = origPos + UDim2.new(0, math.random(-5, 5), 0, 0)})
                    task.wait(0.05)
                end
                Tween(obj, 0.05, {Position = origPos})
            end)
        end
        ...
        local function Verify(input)
            ...
            if type(KeyInfo) == "string" then
                targetKey = KeyInfo
            elseif type(KeyInfo) == "table" then
                -- Support multiple keys or remote keys
                for _, k in pairs(KeyInfo) do
                    if input == k then targetKey = k break end
                end
            end

            -- Remote Check
            if KeySettings.GrabKeyFromSite then
                 -- Logic for remote key would go here, for now using direct match
            end

            if input == targetKey then
                if KeySettings.SaveKey then
                    EnsureFolder("Genim/Keys")
                    WriteFile("Genim/Keys/" .. KeySettings.FileName .. ".txt", input)
                end
                
                Tween(KeySystemFrame, 0.5, {BackgroundTransparency = 1})
                for _, child in pairs(KeySystemFrame:GetChildren()) do
                    if child:IsA("TextLabel") or child:IsA("TextBox") or child:IsA("TextButton") then
                        Tween(child, 0.5, {TextTransparency = 1, BackgroundTransparency = 1})
                    elseif child:IsA("Frame") then
                        Tween(child, 0.5, {BackgroundTransparency = 1})
                        for _, sub in pairs(child:GetChildren()) do
                            if sub:IsA("UIStroke") then Tween(sub, 0.5, {Transparency = 1}) end
                        end
                    end
                end
                task.wait(0.5)
        ...
        GetBtn.MouseButton1Click:Connect(function()
            ...
            if KeySettings.KeyLink then
                ...
                Genim:Notify({
                    Title = "Copiado",
                    Content = "Link da chave copiado para a área de transferência.",
                    Duration = 5
                })
            end
        end)
